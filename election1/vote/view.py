...
handler = RotatingFileHandler('vote_view_log.txt', maxBytes=10_000_000, backupCount=5)
...
def cast(grp_list, token):
    ...
    if not session:
        log_vote_event(' ')
        log_vote_event('--- New Voting Session Started ---'f' for: {grp_list}, ')
    ...
    # validate the groups are valid - the groups along with the token are in the url
        if not are_all_classgrps_valid(grp_list):
            log_vote_event(f"Invalid class group: {grp_list} - Token: {token}")
            home = current_app.config['HOME']
        ...
        if 'error' not in token_list_record:
            if token_list_record.get('grp_list') != grp_list:
                log_vote_event(f"group is bad for: {grp_list} - Token: {token}")
        ...
        if 'error' in token_list_record:
            ...
            log_vote_event(f"Token is good ")
            hashed_token = hashlib.sha256(token.encode('utf-8')).hexdigest()
            session['hashed_token'] = hashed_token
            log_vote_event(f"grp_list matches the value in token_list_record: {grp_list} - hash_token: {session.get('hashed_token', 'Not set')}")
        ...
        if group in (session.get('office_dict')):
            ...
            if g.next_office is None:
                if session['grp_pointer'] + 1 < session['grp_list_length']:
                    ...
                    # log_vote_event('no more offices ')
                    vote_form = ReviewVotes()
                    # print('office_dict ' + str(session.get('office_dict')))
        ...
        for group in office_dict:
            for office in office_dict[group]:
                ...
                for item in office[3]:
                    if item[0] != 99:
                        new_vote = Votes(id_candidate=item[0], votes_token=hashed_token, votes_writein_name=office[4][item_ctr])
                        # new_vote = Votes(id_candidate=item[0], votes_token=token,
                        #                  votes_writein_name=None)
                        db.session.add(new_vote)
                        log_vote_event(f"Vote submitted for {group} {office[0]} {item[0]} - {item[1]}")
                        item_ctr += 1
                    pass
        try:
